...
`rsolver/src/solver/mod.rs` dispatches in order: **same-tiling** (⓪ congruent-tiling prepass) → **compass-part** (⓪b compass+solitary joint partition search; mutually exclusive gate with same-tiling) → **pp-pin** (shape_pattern pre-pin for non-rose `puzzle_piece`; runs before aog because aog's shape catalog OOM-kills the process on exactly this class) → **aog DFS** (direct port of the C++ reference in `third_party/AoG_Solver`) → **rose** (rose_window) → **edge_csp** (edge-variable CSP) → **pieces** (DLX exact cover for shape_pool / area clues / constrained compass) → **backtrack** (region-by-region DFS, disabled by default). The aog solver's internal checks are treated as authoritative (`build_solution_trusted`) — no re-validation in Rust. Full chain docs: `docs/rust-solver/01-总体架构.md`.
