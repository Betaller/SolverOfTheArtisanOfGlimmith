...
结论：`scripts/README.md` 里全量档写的 `-j 8` 在这台 18 核机器上仍然偏激进
（本机常驻 load ~12）。要比对历史数字请用 **`-j 6`**；`-j 8` 的产物只适合当
`--baseline` 快速档自己的对照，不能和 `-j 6` 的数字直接相减。
新基线：`results/bench/20260918_ac31ecb_full.{txt,jsonl}`。
