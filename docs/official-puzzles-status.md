...
| 2026-09-18 | rose_growth 连通性守卫（doc 28） | 直跑 rsolver + 153 题 rose PASS 回归 | 直跑 rsolver | **1144 预估 / 1258** | **+13**（vs 1131） | `try_swap_fix` / `try_chain_move` / `repair_symbol_distribution` 三处搬格点加 `is_connected_set` 守卫（BFS 校验源区域仍连通，否则回滚）。此前 rose_growth 会把区域切成两半，候选被 `validate` 拒绝（25 道 FAIL 显示 `rose:validation_failed`）。修复后 **13 道转 SOLVED**：0879/0390/0660/0972/1156/1222/0634/1403/0417/0697/0776/1392/1150fix。153 道 rose PASS 题 0 回归。 |
