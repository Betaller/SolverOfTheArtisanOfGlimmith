...
- 任何答案都必须通过**独立验证器**（`aog/validate.rs`）才允许上报，防止错误答案“走私”出去。
...
| [08-验证与约束检查](08-验证与约束检查.md) | `constraints.rs` / `aog/validate.rs` / 边界尊重，三套校验各管什么 | ★★☆ | 错误答案怎么被拦下的？ |
...
├── src/
│   ├── main.rs                   # 入口：读 JSON → 建 Puzzle → solve() → 写 JSON
│   ├── types.rs                  # 领域模型：Puzzle/Cell/Edge/Vertex/Shape/...
│   ├── grid.rs                   # 网格工具：连通分量、可填格枚举
