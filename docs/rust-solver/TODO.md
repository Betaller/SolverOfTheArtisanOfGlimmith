...
| P2 | 7. 批量模式（子进程复用） | 中（IO） | 中 | 中 | **已完成**（2026-08-06；2026-08-08 修连带超时局限） |
...
> | 优先级 | 项目 | 收益 | 风险 | 工作量 | 文档 |
> |---|---|---|---|---|---|
> | P1 | 9. 拼块放置候选预过滤（伴生规则感知） | 中 | 低 | 小（~50 行） | [10-拼块优化方向](10-拼块优化方向.md) §4 方向 3 |
> | P1 | 10. Backtrack 利用 shape_pattern 预计算合法格集 | 极高 | 中 | 中（~200 行） | [10-拼块优化方向](10-拼块优化方向.md) §4 方向 1 |
> | P1 | 11. Rose 解除 puzzle_piece 守卫（拼块感知预处理） | 高 | 中 | 中（~300 行） | [../优化/09-rose-puzzle-piece优化调研.md](../优化/09-rose-puzzle-piece优化调研.md) |
> | P2 | 12. 拼块 + 伴生规则混合框架 | 高 | 中 | 大（~800 行） | [10](10-拼块优化方向.md) §4 方向 2 / [../优化/09](../优化/09-rose-puzzle-piece优化调研.md) |
> | P3 | 13. 对称性破缺（polyomino transforms 去重） | 低 | 低 | 极小（~30 行） | [10-拼块优化方向](10-拼块优化方向.md) §4 方向 4 |
...
| 2026-08-08 | timeout 透传修复 + rose clamp 移除（P2 #7 局限闭合） | `main.rs:86`/`io.rs:solve_json_line` 硬编码 `30_000` → `resolve_timeout_ms()` 读 `RSOLVER_TIMEOUT_MS` env（默认 30000，下限 1000），`RustSolver._subprocess_env` 从 `--timeout` 设入；`solve_json_line(line, timeout_ms)` 透传。同步移除 `solver/mod.rs` rose 阶段 `ROSE_TIMEOUT_MS=30_000` clamp（`.min(ROSE_TIMEOUT_MS)` 让 `--timeout 40` 仍封顶 rose 30s）。`RustSolver._wall_budget` 加 `×1.2` slack 吸收 `-j N` CPU 争抢。**P2 #7 连带超时局限随之缓解**（每题 `solve_json_line` 在 `timeout_ms` 停住，串行循环时间有界），`--batch N` 可用于精确验证。`benchmark_rust_solver.py` 新增 `--baseline`(回归模式)/`--zone`/`--skip-slow`/`--skip-slow-threshold`，修复 `--retry-timeouts` 三 bug（结果不写 JSONL、`jobs` 被 adaptive 缩、retry-FAIL 静默）。见 `01`、`scripts/README.md`、`docs/official-puzzles-status.md`。 |
