...
#### 2.1.4 `test_propagator.py` — 约束传播

```
test_forward_checking_simple:
  - 2×2 网格，赋值后邻域候选域正确修剪
...
test_solitary_propagation:
  - 独居规则下，含符号的格禁止与其他含符号格同区

test_compass_propagation:
  - 罗盘 4 方向长度限制
  - 某方向长度达到标注值时，下一格必须异色

test_brick_ring_propagation:
  - 砖纹避免 4 条边交于一点
  - 环纹避免 3 条边交于一点
```

#### 2.1.5 `test_backtrack.py` — 回溯搜索

```
test_backtrack_simple_2x2:
  - 2×2 无约束，能找到划分
  - 搜索结果非空，所有格已分配

test_backtrack_trivial_1_region:
  - 2×2 整个网格为一个区域
  - 唯一解

test_backtrack_mrv_heuristic:
  - MRV 选择候选域最少的格
  - MRV 相同时度启发式选择约束度最高格

test_backtrack_region_growth:
  - 从种子格开始区域生长
  - 连通性约束正确维护

test_backtrack_symmetry_breaking:
  - 第一个区域固定编号为 0
  - 避免颜色标签置换的对称解

test_backtrack_island_detection:
  - 提前检测未分配区域形成的孤岛
  - 孤岛面积不符时立即回溯
```

#### 2.1.6 `test_validator.py` — 解验证器

```
test_validator_empty_puzzle:
  - 无规则时任何划分都验证通过

test_validator_shape_pool:
  - 形状匹配时验证通过
  - 形状不匹配时验证失败

test_validator_rose_window:
  - 满足玫瑰窗条件时通过
  - 区域数 ≠ 每种符号数量时失败
  - 某区域缺少符号时失败
  - 符号未被覆盖时失败

test_validator_heterogeneous:
  - 异生标记两侧形状不同时通过
  - 异生标记两侧形状相同时失败

test_validator_homogeneous:
  - 双生标记两侧形状相同时通过
  - 双生标记两侧形状不同时失败

test_validator_precise:
  - 所有区域面积 = 给定值通过
  - 任一区域面积不符则失败

test_validator_puzzle_piece:
  - 拼块标记格区域形状匹配通过
  - 不匹配时失败
...
test_validator_range:
  - 所有区域面积在范围内通过
  - 超出范围失败

test_validator_fence:
  - 边界分布匹配图案通过
  - 不匹配失败

test_validator_different:
  - 所有区域形状不同通过
  - 存在相同形状失败

test_validator_solitary:
  - 每区域 1 符号通过
  - 某区域 0 或 2+ 符号失败
...
test_validator_non_block:
  - 无非矩形通过
  - 存在矩形失败

test_validator_differentiation:
  - 相邻区域面积不同通过
  - 存在相同面积相邻失败

test_validator_brick:
  - 无四边交于一点通过
  - 存在四边交点失败
...
test_validator_inequality:
  - 箭头指向侧面积 < 箭尾侧通过
  - 指向侧 ≥ 箭尾侧失败

test_validator_difference:
  - 边数字 = 面积差通过
  - 不匹配失败

test_validator_watchtower:
  - 顶点数字 = 不同区域数通过
  - 不匹配失败

test_validator_compass:
  - 4 方向距离匹配通过
  - 任一方向不匹配失败
  - -1 方向不检查通过

test_validator_multiple_rules:
  - 多条规则同时验证
  - 全部通过才通过
  - 任一失败即失败

test_validator_solution_roundtrip:
  - 已验证的解再次验证仍通过
```
