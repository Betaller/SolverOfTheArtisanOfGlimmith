...
当前 147 道 FAIL（1111/1258 PASS），基于 `20260904_nobacktrack_full.jsonl` 实测：

| 类别 | 数量 | 根因 | 优化方向 |
|---|---|---|---|
| **无规则纯分区** | **134** | 无约束引导搜索，aog 超时 | 并行化/搜索启发式 |
| rose_window 单规则 | 6 | rose 候选爆炸 | rose 预算优化 |
| homogeneous+puzzle_piece+rose_window | 3 | 多规则组合 | 传播补齐 |
| 其他有规则组合 | 4 | 各异 | 逐案分析 |

**关键发现**：134/147（91%）的 FAIL 题目**没有任何规则**（空 rules 列表）。这些是纯区域分割谜题，求解器没有任何约束可利用，只能靠搜索穷举。
...
134 道无规则 FAIL 题是最大的 FAIL 类别。这些题目的特点是：
- 没有任何约束线索（无 area/compass/watchtower/ring/brick 等）
- 求解器只能靠搜索穷举所有可能的分区
- aog 超时（15s/40s 内无法找到解）

**可能的优化方向**：

1. **并行化**（P3）：多个求解器并发运行，先赢者取消其余。无规则题可能被不同求解器以不同搜索顺序解出。

2. **搜索启发式改进**：
   - **MRV + 度启发式**：优先选择度最低的未确定边（减少分支因子）
   - **LCV 值序**：优先尝试 Uncut（保持连通性），因为大多数有效分区是连通的
   - **随机重启**：如果搜索卡住，随机打乱未确定边的顺序重新开始

3. **连通性剪枝**：
   - 在搜索过程中，如果某个组件被 Cut 边完全包围但面积不满足任何合理约束 → 矛盾
   - 如果剩余格数不足以形成足够多的连通区域 → 矛盾

4. **对称性破缺**：
   - 对于无规则题，很多分区是对称等价的
   - 可以强制某些边的状态来打破对称（例如强制左上角的边为 Uncut）

5. **专用无规则求解器**：
   - 对于纯分区问题，可以使用专门的算法（如递归二分法、strip 分解等）
   - 但实现复杂度高，且不一定比通用搜索快

**预计收益**：并行化可能 +5~10 道，搜索启发式可能 +3~5 道。合计 +8~15 道。
