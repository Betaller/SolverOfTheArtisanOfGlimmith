...
def make_and_save(category: str, filename: str, puzzle: Puzzle) -> bool:
    ...
    if sol.solved:
        ...
        print(
            f"  OK {category}/{filename}: {len(sol.regions)}区域 "
            f"面积{dict(areas)} {sol.elapsed_ms}ms"
        )
...
for r, c, s in syms_map:
    b.cell(r, c).symbol = s
make_and_save(
    "rose_window",
    "rose_02_5x5_3sym_blocked.json",
...
make_and_save(
    "rose_window",
    "rose_03_6x4_2sym_boundary.json",
    Puzzle.from_board(b, rules=[Rule.rose_window(["A", "B"])]),
)
...
make_and_save(
    "shape_pool",
    "pool_03_6x4_domino.json",
    Puzzle.from_board(Board(6, 4), rules=[Rule.shape_pool([dom_h, dom_v])]),
)

# P4: 5x5, 池={L形3, 直线3, T形4}, 中间blocked
b = Board(5, 5)
b.cell(2, 2).blocked = True
tri_l = Shape(cells=frozenset([(0, 0), (1, 0), (1, 1)]))
tri_s = Shape(cells=frozenset([(0, 0), (0, 1), (0, 2)]))
tri_t = Shape(cells=frozenset([(0, 0), (0, 1), (0, 2), (1, 1)]))
make_and_save(
    "shape_pool",
    "pool_04_5x5_triomino.json",
    Puzzle.from_board(b, rules=[Rule.shape_pool([tri_l, tri_s, tri_t])]),
)
...
for r in range(4):
    for c in range(4):
        b.cell(r, c).symbol = "A" if (r + c) % 2 == 0 else "B"
make_and_save(
    "rose_window+shape_pool",
    "combo_01_4x4_rose_domino.json",
    Puzzle.from_board(
        b,
        rules=[
...
make_and_save(
    "rose_window+shape_pool",
    "combo_04_6x6_rose+pool_boundary.json",
    Puzzle.from_board(
        b,
        rules=[
