...
def _compass_to_dict(compass: CompassClue) -> dict[str, Any]:
    return {
        "up": compass.up,
        "down": compass.down,
        "left": compass.left,
...
def _cell_to_dict(c: Cell) -> dict[str, Any]:
    ...
    if c.compass is not None:
        entry["compass"] = _compass_to_dict(c.compass)
...
def puzzle_to_dict(puzzle: Puzzle) -> dict[str, Any]:
    cells_data = [_cell_to_dict(c) for c in puzzle.cells]
    edges_data = [_edge_to_dict(e) for e in puzzle.edges]
    vertices_data = [_vertex_to_dict(v) for v in puzzle.vertices]
...
def _apply_cell_data(board: Board, cdata: dict[str, Any]) -> None:
...
def _apply_edge_data(board: Board, edata: dict[str, Any]) -> None:
    ...
    if e is None:
        raise PuzzleFormatError(
            f"无效的边框坐标: ({edata['r1']},{edata['c1']})-({edata['r2']},{edata['c2']})"
        )
...
def _apply_vertex_data(board: Board, vdata: dict[str, Any]) -> None:
...
def _collect_outer_boundaries(data: dict[str, Any]) -> list[tuple[int, int, int, int]]:
    raw: list[tuple[int, int, int, int]] = []
    for ob in data.get("outer_boundaries", []):
        raw.append((int(ob["r1"]), int(ob["c1"]), int(ob["r2"]), int(ob["c2"])))
    return [k for k in map(_canonical_outer, raw) if k is not None]
...
def dict_to_puzzle(data: dict[str, Any]) -> Puzzle:
    try:
        ...
        raise PuzzleFormatError(f"无效的网格尺寸: {e}") from e

    board = Board(height, width)

    for cdata in data.get("cells", []):
        _apply_cell_data(board, cdata)
    for edata in data.get("edges", []):
        _apply_edge_data(board, edata)
    for vdata in data.get("vertices", []):
        _apply_vertex_data(board, vdata)

    rules = [rule_from_dict(r) for r in data.get("rules", [])]
    shape_pool = [shape_from_dict(s) for s in data.get("shape_pool", [])]
    outer_boundaries = _collect_outer_boundaries(data)
    return Puzzle(
        height=height,
        width=width,
        cells=board.cells(),
